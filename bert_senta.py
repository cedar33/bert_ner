...
class NerProcessor(DataProcessor):
  ...
  def get_labels(self):
    ...
    return ["contradiction","negative", "positive"]
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
        ...
        label_t = tokenization.convert_to_unicode(line[1])
        # tf.logging.info(text_a)
        # tf.logging.info(label_t)
        labels = list()
        if set_type == "test":
            # print("*****test*****")
  ...
  def model_fn(features, mode, params):  # pylint: disable=unused-argument
    ...
    if not mode == tf.estimator.ModeKeys.PREDICT:
      ...
      def metric_fn(seq_length, max_len, label_ids, pred_ids):
        ...
        metrics = {
            'acc': tf.metrics.accuracy(label_ids_metric, pred_ids),
            'precision': precision(label_ids_metric, pred_ids, params['num_labels'], indices),
    ...
    d = tf.data.Dataset.from_tensor_slices({
        ...
        "segment_ids":
            tf.constant(
                ...
                dtype=tf.int32),
        "label_ids":
            tf.constant(all_label_ids, shape=[num_examples, 3], dtype=tf.int32),
    })
